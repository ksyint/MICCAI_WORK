...
class MultiModalProjector(nn.Module):
    def __init__(self, input_size, output_size, mlp_depth, proj_out_num=256):
        ...
        self.mm_projector = nn.Sequential(
            ...
            *[nn.Sequential(nn.GELU(), nn.Linear(output_size, output_size))
...
class LowHighHybridMLP(nn.Module):
    def __init__(self, low_input_size, high_input_size, output_size,
                 mlp_depth, proj_out_num=288):
        super().__init__()
        self.proj_out_num = proj_out_num
...
class MixerLowHighHybridMLP(nn.Module):
    def __init__(self, low_input_size=(256, 384), low_output_size=[192, 128],
                 high_input_size=(32, 768), high_output_size=[64, 128],
                 output_dim=3584, depth=2, mlp_depth=2, proj_out_num=256):
        super().__init__()
        self.proj_out_num = proj_out_num
        self.low_mixer = nn.ModuleList([
            MixerLayer(
                input_size=((low_output_size[i - 1], output_dim // (2 ** (depth - i)))
                            if i > 0 else low_input_size),
                output_size=(low_output_size[i], output_dim // (2 ** (depth - i - 1))),
                mlp_depth=mlp_depth,
            ) for i in range(depth)
        ])
        self.high_mixer = nn.ModuleList([
            MixerLayer(
                input_size=((high_output_size[i - 1], output_dim // (2 ** (depth - i)))
                            if i > 0 else high_input_size),
                output_size=(high_output_size[i], output_dim // (2 ** (depth - i - 1))),
                mlp_depth=mlp_depth,
            ) for i in range(depth)
        ])
