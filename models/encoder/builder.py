...
from .dcformer import decomp_small, decomp_tiny, decomp_nano
    ...
    def __init__(self, config):
        ...
        if config.vision_tower == "vit3d":
            self.vision_tower = Vit3D(
                input_size=config.input_size, dim=config.dim, depth=config.depth)
        elif config.vision_tower == "dcformer":
            self.vision_tower = decomp_small(input_size=config.input_size)
            self.low_input_size = self.vision_tower.channels[-2]
            self.high_input_size = self.vision_tower.channels[-1]
        elif config.vision_tower == "dcformer_tiny":
            self.vision_tower = decomp_tiny(input_size=config.input_size)
            self.low_input_size = self.vision_tower.channels[-2]
            self.high_input_size = self.vision_tower.channels[-1]
        else:
