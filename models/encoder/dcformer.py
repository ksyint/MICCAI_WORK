...
class DecompConv3D(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride=1, groups=1, norm=True, act=None):
        super().__init__()
        self.act = act
        self.c1 = nn.Sequential(
            nn.Conv3d(in_dim, out_dim, kernel_size=(kernel_size, 1, 1),
                      padding=(kernel_size // 2, 0, 0), stride=stride, groups=groups),
            nn.BatchNorm3d(out_dim) if norm else nn.Identity(),
        )
        self.c2 = nn.Sequential(
            nn.Conv3d(in_dim, out_dim, kernel_size=(1, kernel_size, 1),
                      padding=(0, kernel_size // 2, 0), stride=stride, groups=groups),
            nn.BatchNorm3d(out_dim) if norm else nn.Identity(),
        )
        self.c3 = nn.Sequential(
            nn.Conv3d(in_dim, out_dim, kernel_size=(1, 1, kernel_size),
                      padding=(0, 0, kernel_size // 2), stride=stride, groups=groups),
            nn.BatchNorm3d(out_dim) if norm else nn.Identity(),
        )
...
class ConvPosEnc(nn.Module):
    def __init__(self, dim, k=3, decompose=False):
        ...
        if decompose:
            ...
            self.proj = nn.Conv3d(dim, dim, to_3tuple(k), to_3tuple(1), to_3tuple(k // 2), groups=dim)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, size=None):
        B_, N, C = x.shape
        qkv = (self.qkv(x).reshape(B_, N, 3, self.head_num, C // self.head_num)
               .permute(2, 0, 3, 1, 4))
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = (self.qkv(x).reshape(B, N, 3, self.head_num, C // self.head_num)
               .permute(2, 0, 3, 1, 4))
        q, k, v = qkv[0], qkv[1], qkv[2]
        k = k * self.scale
...
class TransformerBlock(nn.Module):
    def __init__(self, inp, oup, image_size, kernel_size, heads=8, dim_head=32,
                 downsample=False, dropout=0.0):
        super().__init__()
...
class ConvBlock(nn.Module):
    def __init__(self, inp, oup, image_size, kernel_size=7, downsample=False, expansion=4):
        super().__init__()
        self.downsample = downsample
        ...
        self.scale = (nn.Parameter(layer_scale_init_value * torch.ones((oup)), requires_grad=True)
...
class Encoder(nn.Module):
    def __init__(self, input_size, in_channels, num_blocks, channels,
                 kernel_sizes=[7, 7, 7, 7], block_types=["C", "C", "C", "C"]):
        super().__init__()
        self.dims = channels
        ih, iw, it = input_size
        block = {"C": ConvBlock, "T": TransformerBlock}
        i = 4
        self.s0 = self._make_layer(DecomposedStem, in_channels, channels[0],
                                    num_blocks[0], kernel_sizes[0], (ih // i, iw // i, it // i))
        self.s1 = self._make_layer(block[block_types[0]], channels[0], channels[1],
                                    num_blocks[1], kernel_sizes[0],
                                    (ih // (i * 2), iw // (i * 2), it // (i * 2)))
        self.s2 = self._make_layer(block[block_types[1]], channels[1], channels[2],
                                    num_blocks[2], kernel_sizes[1],
                                    (ih // (i * 4), iw // (i * 4), it // (i * 4)))
        self.s3 = self._make_layer(block[block_types[2]], channels[2], channels[3],
                                    num_blocks[3], kernel_sizes[2],
                                    (ih // (i * 8), iw // (i * 8), it // (i * 8)))
        self.s4 = self._make_layer(block[block_types[3]], channels[3], channels[4],
                                    num_blocks[4], kernel_sizes[3],
                                    (ih // (i * 16), iw // (i * 16), it // (i * 16)))
...
class DecompModel(nn.Module):
    def __init__(self, input_size=(256, 256, 128), in_channels=1,
                 num_blocks=[2, 2, 3, 5, 2], channels=[64, 96, 192, 384, 768],
                 kernel_sizes=[13, 11, 9, 7], block_types=["C", "C", "C", "C"]):
        super().__init__()
        self.channels = channels
        self.encoder = Encoder(input_size, in_channels, num_blocks, channels,
                               kernel_sizes, block_types)
...
def decomp_small(input_size=(256, 256, 128)):
    return DecompModel(input_size=input_size, num_blocks=[1, 2, 3, 6, 2],
                       channels=[64, 96, 192, 384, 768])


def decomp_tiny(input_size=(256, 256, 128)):
    return DecompModel(input_size=input_size, num_blocks=[1, 2, 3, 3, 2],
                       channels=[64, 96, 192, 384, 768])


def decomp_nano(input_size=(256, 256, 128)):
    return DecompModel(input_size=input_size, num_blocks=[1, 1, 1, 1, 1],
